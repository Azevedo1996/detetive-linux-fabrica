...
class Mission(models.Model):
    ...
    expected_answer = models.CharField('Resposta esperada', max_length=180)
    aliases = models.CharField('Respostas aceitas', max_length=500, blank=True, help_text='Separe por vírgula.')
    command_example = models.CharField('Exemplo de comando', max_length=260, blank=True)
    explanation = models.TextField('Explicação')
    reward = models.CharField('Recompensa da missão', max_length=180, blank=True)
...
class PlayerSession(models.Model):
    player_name = models.CharField('Nome do detetive', max_length=80, blank=True)
    ...
    def __str__(self):
        return f'{self.player_name or "Detetive"} - Fase {self.selected_phase}'

    def add_reward(self, reward):
        if not reward: return
        values = [r for r in self.rewards.split('\n') if r.strip()]
        if reward not in values:
